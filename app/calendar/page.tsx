...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ view?: string; date?: string }> }) {
  ...
  return <main className="life-app-shell"><AppNavigation active="calendar" /><CalendarExperience view={view} selected={selected} today={today} items={data.items} /><QuickAdd defaultDate={selected} /></main>;
}
