...
export default async function ShoppingListPage() {
  ...
  const [plansResult,offers]=await Promise.all([supabase.from("daily_meal_plans").select("plan_date,menu_name,selections").gte("plan_date",today).order("plan_date"),getPromotions()]); const plans=plansResult.data; const menus=getMenuLibrary(user.user_metadata as MealMenuSettings); const items=addPromotions(addPriceEstimates(collect((plans??[]) as {menu_name:string;selections:Record<string,number>}[],menus),userProducts(user.user_metadata as Record<string,unknown>)),offers); const estimatedTotal=items.reduce((sum,item)=>sum+(item.promotionEstimate??item.estimatedPrice??0),0); const stores=new Map<string,{count:number,total:number}>(); for(const item of items)if(item.promotion){const current=stores.get(item.promotion.store)??{count:0,total:0};current.count+=1;current.total+=item.promotionEstimate??item.promotion.price;stores.set(item.promotion.store,current);}
  return <main className="life-app-shell"><AppNavigation active="shopping" /><section className="shopping-page"><p className="life-kicker">От днес нататък</p><h1>Пазарски списък</h1><p className="shopping-range">Всички отбелязани хранения от {today}, независимо от седмицата.</p><div className="shopping-summary"><strong>{plans?.length ?? 0}</strong><span>планирани дни</span><strong>{items.length}</strong><span>различни продукта</span>{estimatedTotal>0?<><strong>{money(estimatedTotal)}</strong><span>ориентировъчно</span></>:null}</div>{stores.size?<div className="shopping-store-plan"><strong>Къде е най-изгодно</strong><div>{[...stores.entries()].sort((a,b)=>b[1].count-a[1].count).map(([store,summary])=><span key={store}><b>{store}</b><small>{summary.count} продукта · около {money(summary.total)}</small></span>)}</div></div>:null}<div className="shopping-list">{items.length ? items.map(item=><label key={`${item.name}-${item.unit ?? ""}`}><input type="checkbox"/><span>{item.name}{item.promotion?<a className="shopping-promotion" href={item.promotion.url} target="_blank" rel="noreferrer"><b>Най-изгодно в {item.promotion.store}</b><em>{item.promotionPackages&&item.promotionPackages>1?`${item.promotionPackages} оп. · `:""}{money(item.promotionEstimate??item.promotion.price)} · до {item.promotion.validUntil} ↗</em></a>:item.estimatedPrice?<small>Около {money(item.estimatedPrice)} по твоя последна цена</small>:null}</span><b>{amountLabel(item)}</b></label>) : <p>Първо избери меню и варианти за днешна или бъдеща дата в календара.</p>}</div></section></main>;
}
