...
export default async function ShoppingListPage() {
  ...
  const {data:plans}=await supabase.from("daily_meal_plans").select("plan_date,menu_name,selections").gte("plan_date",today).order("plan_date"); const menus=getMenuLibrary(user.user_metadata as MealMenuSettings); const items=collect((plans??[]) as {menu_name:string;selections:Record<string,number>}[],menus);
