...
## ADR-007 — Единна календарна проекция

**Статус:** Прието, 2026-08-14

**Решение:** Източниците се преобразуват към общ `CalendarItem` само при четене.
**Защо:** Бъдещи модули могат да се виждат в календара без универсална таблица и двустранна синхронизация.

## ADR-008 — Правила за повторение

**Статус:** Прието, 2026-08-14

**Решение:** Пазим правило, а срещанията се разгръщат за заявения диапазон.
**Защо:** Няма безкрайно генериране на редове и редактирането остава предвидимо.

## ADR-009 — Локална дата и UTC момент

**Статус:** Прието, 2026-08-14

**Решение:** Целодневните записи използват `date`; часовите — `timestamptz` плюс IANA зона.
**Защо:** Рождените дни не се изместват, а часовите събития остават точни при пътуване и лятно време.
