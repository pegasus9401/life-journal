  ...
  return <section className="assistant-layout">
    ...
    <div className="assistant-panel">
      ...
      <form className="assistant-composer" onSubmit={submit}>
        ...
        <div className="assistant-input-row"><label className="assistant-camera" title="Снимай или избери продукт"><input type="file" accept="image/*" capture="environment" onChange={(event) => void attachPhoto(event)} disabled={pending} /><span aria-hidden="true">📷</span><b className="sr-only">Снимай или избери продукт</b></label><textarea id="assistant-command" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(input, image); } }} placeholder="Напиши команда или снимай продукт…" rows={2} maxLength={8000} disabled={pending} /><button type="submit" disabled={pending || (!input.trim() && !image)} aria-label="Изпрати">↑</button></div>
