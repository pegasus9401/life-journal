  ...
  return <section className="adaptive-month is-expanded" aria-label="Месечен календар">
    <div className="month-calendar">
      ...
      <div className="month-grid">{days.map(day => {
        const dayItems = itemsOn(items, day); const stickers = dayItems.filter(item => item.category === "sticker"); const visibleDayItems = dayItems.filter(item => item.category !== "sticker"); const plan = mealOn(mealPlans, day); const workouts = workoutsOn(workoutPlans, day); const outside = day.slice(0, 7) !== selected.slice(0, 7);
