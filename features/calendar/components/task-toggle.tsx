...
export function TaskToggle({ id, completed }: { id: string; completed: boolean }) {
  ...
  return <button className={`task-toggle ${value ? "completed" : ""}`} type="button" aria-label={value ? "Маркирай като незавършена" : "Маркирай като завършена"} aria-pressed={value} disabled={pending} onClick={async () => { setPending(true); setValue(!value); const result = await toggleTask(id, !value); if (!result.ok) setValue(value); setPending(false); router.refresh(); }}><span aria-hidden="true">{value ? "✓" : ""}</span></button>;
}
