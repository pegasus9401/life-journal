...
import { splitStickerDescription } from "./stickers";
...
export function aggregateCalendarItems(events: CalendarEventRow[], tasks: TaskRow[], birthdays: BirthdayRow[], rangeStart: string, rangeEnd: string): CalendarItem[] {
  ...
  for (const event of events) {
    const sticker = splitStickerDescription(event.description).sticker || (event.category === "sticker" ? event.title : "");
  ...
  for (const task of tasks) {
    ...
    for (const occurrence of expandRecurrence(task.due_date, task.recurrence_kind, task.recurrence_interval, rangeStart, rangeEnd, task.recurrence_end)) {
      items.push({ id: `task:${task.id}:${occurrence}`, type: "task", sourceId: task.id, sourceType: "tasks", title: task.title, date: occurrence, time: task.due_time?.slice(0, 5), allDay: !task.due_time, completed: task.completed, category: task.category, color: task.priority === "high" ? "rose" : "indigo", sticker, recurring: task.recurrence_kind !== "none" });
    }
  }
