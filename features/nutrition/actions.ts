...
export async function saveNutritionGoals(_state: NutritionActionState, formData: FormData): Promise<NutritionActionState> {
  ...
  if (!user) return fail("Сесията изтече.");
  const value = parsed.data;
  const { error } = await supabase.from("nutrition_goals").upsert({ owner_id: user.id, calorie_goal: value.calories, protein_goal: value.protein, carbs_goal: value.carbs, fat_goal: value.fat }, { onConflict: "owner_id" });
  if (error) return fail("Целите не можаха да бъдат запазени.");
  revalidatePath("/nutrition");
