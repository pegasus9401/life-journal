...
export function DayMealPlanner({ date, initialMenu = "Меню 1", initialSelections = {}, initialHasPlan = false }: { date: string; initialMenu?: string; initialSelections?: Record<string, number>; initialHasPlan?: boolean }) {
  ...
  const persist = useCallback((nextMenu: string, nextSelections: Record<string, number>, refresh = true) => startTransition(async () => {
  ...
  useEffect(() => {
    if (!dirty.current || !meals) return;
    if (saveTimer.current) clearTimeout(saveTimer.current);
    saveTimer.current = setTimeout(() => { dirty.current = false; persist(menu, selections); }, 350);
    return () => { if (saveTimer.current) clearTimeout(saveTimer.current); };
  }, [meals, menu, persist, selections]);

  const changeMenu = (value: string) => { dirty.current = true; setMenu(value); setSelections({}); setMessage("Ще се запази автоматично…"); };
  const changeSelection = (meal: string, value: number) => { dirty.current = true; setSelections(current => ({ ...current, [meal]: value })); setMessage("Ще се запази автоматично…"); };
  const saveNow = () => {
    dirty.current = false;
    if (saveTimer.current) clearTimeout(saveTimer.current);
  ...
  return <section className="day-meal-planner">
    ...
    <div className="day-meal-planner-actions"><button className="primary-button" type="button" disabled={pending || !activeNames.includes(menu)} onClick={saveNow}>{pending ? "Запазване…" : hasPlan ? "Запази промените" : "Добави към деня"}</button>{hasPlan ? <button className="remove-meal-plan" type="button" disabled={pending} onClick={removePlan}>Премахни от този ден</button> : null}</div>
