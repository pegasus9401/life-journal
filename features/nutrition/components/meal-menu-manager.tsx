...
export function MealMenuManager() {
  ...
  return <section className="meal-plan managed-meal-plan">
    ...
    {!creating && selectedName && selectedData ? <>
      ...
      <div className="meal-plan-list managed-meal-details">{orderedMealEntries(selectedData).map(([meal, variants]) => {
        ...
        return <article className="meal-plan-card" key={meal}>
          <header className="meal-plan-card-header"><div className="meal-plan-title"><span className="meal-plan-icon">✦</span><div><h3>{meal}</h3><span>{variants.length} {variants.length === 1 ? "вариант" : "варианта"}</span></div></div><div className="meal-plan-card-macros"><span>kcal <b>{selectedNutrition.meals[meal]?.kcal ?? 0}</b></span><span>П <b>{selectedNutrition.meals[meal]?.protein ?? 0}</b></span><span>В <b>{selectedNutrition.meals[meal]?.carbs ?? 0}</b></span><span>М <b>{selectedNutrition.meals[meal]?.fat ?? 0}</b></span></div></header>
    ...
    {creating ? <div className="meal-menu-create managed-meal-editor">
      ...
      <fieldset className="nutrition-values-editor"><legend>Общо за деня</legend><label>Калории<input type="number" min="0" step="1" value={menuNutrition.kcal} onChange={event => updateMenuNutrition("kcal", event.target.value)} /></label><label>Протеин (г)<input type="number" min="0" step="0.1" value={menuNutrition.protein} onChange={event => updateMenuNutrition("protein", event.target.value)} /></label><label>Въглехидрати (г)<input type="number" min="0" step="0.1" value={menuNutrition.carbs} onChange={event => updateMenuNutrition("carbs", event.target.value)} /></label><label>Мазнини (г)<input type="number" min="0" step="0.1" value={menuNutrition.fat} onChange={event => updateMenuNutrition("fat", event.target.value)} /></label></fieldset>
      ...
      <div className="meal-menu-fields">{mealNames.map(meal => <section className="meal-variants-editor" key={meal}>
        ...
        <fieldset className="nutrition-values-editor meal-nutrition-values"><legend>Макроси за храненето</legend><label>kcal<input type="number" min="0" step="1" value={menuNutrition.meals[meal]?.kcal ?? 0} onChange={event => updateMealNutrition(meal, "kcal", event.target.value)} /></label><label>Протеин<input type="number" min="0" step="0.1" value={menuNutrition.meals[meal]?.protein ?? 0} onChange={event => updateMealNutrition(meal, "protein", event.target.value)} /></label><label>Въглехидрати<input type="number" min="0" step="0.1" value={menuNutrition.meals[meal]?.carbs ?? 0} onChange={event => updateMealNutrition(meal, "carbs", event.target.value)} /></label><label>Мазнини<input type="number" min="0" step="0.1" value={menuNutrition.meals[meal]?.fat ?? 0} onChange={event => updateMealNutrition(meal, "fat", event.target.value)} /></label></fieldset>
        <div className="meal-variant-list">{meals[meal].map((variant, index) => <div className="meal-variant-row" key={index}>
          <div className="meal-variant-products">
            ...
            {(variantProducts(variant).length ? variantProducts(variant) : [""]).map((product, productIndex, products) => {
              ...
              const unitSuggestionsId = `unit-suggestions-${mealNames.indexOf(meal)}-${index}-${productIndex}`;
              ...
              return <div className="meal-product-row" key={productIndex}>
                ...
                <div className="product-unit-combobox">
                  <input className="product-unit-input" role="combobox" value={unitValue} onFocus={() => { setUnitDrafts(current => ({ ...current, [unitFieldKey]: unitValue })); setActiveUnitField(unitFieldKey); }} onChange={event => { const value = event.target.value; setUnitDrafts(current => ({ ...current, [unitFieldKey]: value })); updateProductField(meal, index, productIndex, "unit", value); setActiveUnitField(unitFieldKey); }} onKeyDown={event => { if (event.key === "Escape") setActiveUnitField(null); }} onBlur={() => window.setTimeout(() => setActiveUnitField(null), 120)} placeholder="Мерна единица" autoComplete="off" aria-label={`Мерна единица за продукт ${productIndex + 1}`} aria-autocomplete="list" aria-controls={unitSuggestionsId} aria-expanded={activeUnitField === unitFieldKey} />
                  {activeUnitField === unitFieldKey ? <div id={unitSuggestionsId} className="product-unit-suggestions" role="listbox">{unitSuggestions(unitValue).length ? unitSuggestions(unitValue).map(([value, label]) => <button type="button" role="option" aria-selected={value === unitValue} key={value} onMouseDown={event => event.preventDefault()} onClick={() => { setUnitDrafts(current => ({ ...current, [unitFieldKey]: value })); updateProductField(meal, index, productIndex, "unit", value); setActiveUnitField(null); }}><strong>{value}</strong><span>{label}</span></button>) : <p>Няма предложение - можеш да използваш въведения текст.</p>}</div> : null}
                </div>
