...
type MealRow = { id: string; meal_date: string; name: string; planned_time: string | null; position: number; legacy_payload: Record<string, unknown> | null };
...
export const getDynamicNutritionDay = cache(async (date: string): Promise<DynamicNutritionData | null> => {
  ...
  const [mealsResult, itemsResult, templatesResult, templateMealsResult, library] = await Promise.all([
    supabase.from("day_meals").select("id, meal_date, name, planned_time, position, legacy_payload").eq("owner_id", user.id).eq("meal_date", date).order("position"),
  ...
  const meals: DynamicMeal[] = ((mealsResult.data ?? []) as MealRow[]).map((meal) => ({
    id: meal.id, date: meal.meal_date, name: meal.name, plannedTime: meal.planned_time?.slice(0, 5) ?? null, position: meal.position, legacyPayload: meal.legacy_payload,
