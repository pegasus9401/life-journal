...
export function recommendNutrition(profile: Profile | null, goal = profile?.fitness_goal ?? null, today = new Date(), targetWeightKg = profile?.target_weight_kg ?? null): NutritionRecommendation | null {
  ...
  return { calories, protein, carbs, fat, age, restingCalories: Math.round(restingEnergy), maintenanceCalories, adjustmentCalories, activityLabel: activityLabel[profile.activity_level], goalLabel, estimatedWeeklyChangeKg, estimatedWeeks };
}
