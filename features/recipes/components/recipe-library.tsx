...
export function RecipeLibrary({ initialRecipes, products }: { initialRecipes: Recipe[]; products: FoodProduct[] }) {
  ...
  return <section className={styles.page}>
    ...
    {draft ? <div className={styles.backdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) setDraft(null); }}><section className={styles.editor} role="dialog" aria-modal="true" aria-label="Редактиране на рецепта">
      ...
      <section className={styles.ingredientsEditor}><header><div><h3>Продукти в рецептата</h3><p>Всеки продукт е отделен и участва в изчислението според грамажа.</p></div><div className={styles.ingredientActions}><Link href="/products">Моите продукти</Link><button type="button" onClick={addIngredient} disabled={!products.length || draft.ingredients.length >= products.length}>+ Добави продукт</button></div></header>{draft.ingredients.map((ingredient, index) => { const product = productIndex.get(ingredient.productId); const factor = ingredient.grams / 100; return <article className={styles.ingredientCard} key={ingredient.id}><div className={styles.ingredientRow}><label>Продукт<select value={ingredient.productId} onChange={(event) => updateIngredient(index, { productId: event.target.value })}>{products.map((option) => <option key={option.id} value={option.id} disabled={draft.ingredients.some((item, itemIndex) => itemIndex !== index && item.productId === option.id)}>{option.name}{option.brand ? ` · ${option.brand}` : ""} · {round(option.calories100g)} kcal/100 g</option>)}</select></label><label>Грамове<input type="number" min="0.01" max="100000" step="0.1" value={ingredient.grams} onChange={(event) => { const grams = Math.max(.01, Number(event.target.value) || .01); updateIngredient(index, { grams, quantity: grams, unit: "g" }); }} /></label><button type="button" onClick={() => setDraft({ ...draft, ingredients: draft.ingredients.filter((_, itemIndex) => itemIndex !== index) })} aria-label="Премахни съставката">×</button></div>{product ? <div className={styles.ingredientNutrition}><span><b>{round(product.calories100g * factor)}</b> kcal за {ingredient.grams} g</span><span>П {round(product.protein100g * factor)} g</span><span>В {round(product.carbs100g * factor)} g</span><span>М {round(product.fat100g * factor)} g</span><small>За 100 g: {round(product.calories100g)} kcal · П {round(product.protein100g)} · В {round(product.carbs100g)} · М {round(product.fat100g)}</small></div> : null}</article>; })}{!products.length ? <div className={styles.noProducts}>Нямаш добавени продукти. <Link href="/products">Добави продукт</Link>, за да създадеш точна рецепта.</div> : null}</section>
