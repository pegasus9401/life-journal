...
export function LifeTimeline({ items, calories, calorieGoal, protein, proteinGoal }: { items: TimelineItem[]; calories: number; calorieGoal: number; protein: number; proteinGoal: number }) {
  ...
  return <section className={styles.timeline} aria-labelledby="timeline-title">
    <header><div><p>ДНЕВНА ЛИНИЯ</p><h2 id="timeline-title">Как минава денят</h2></div><span>{visible.length} записа</span></header>
    ...
    </article>)}{!visible.length ? <div className={styles.timelineEmpty}><strong>Денят още е празен</strong><span>Добави храна, задача, тренировка или спомен с бутона +.</span></div> : null}</div>
  </section>;
}
