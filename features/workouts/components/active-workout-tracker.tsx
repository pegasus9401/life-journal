    ...
    {!expanded ? <aside className="active-workout-dock" aria-label="Активна тренировка">
      ...
      <div className="active-workout-exercises">
        {active.exercises.map((exercise, exerciseIndex) => {
          ...
          return <details key={exercise.id} open={exerciseDone < exercise.results.length && active.exercises.slice(0, exerciseIndex).every((item) => item.results.every((result) => result.done))}>
